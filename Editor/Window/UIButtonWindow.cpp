...
{
	CEditorLabel* label = CreateWidget<CEditorLabel>("UI 버튼");
	...
	m_ButtonEnableCheck = CreateWidget<CEditorCheckBox>("활성화 버튼");
	...
	pos = CreateWidget<CEditorCursorPos>("Pos");
	pos->SetPosY(-2.5f);
	button = CreateWidget<CEditorButton>("사운드 변경", 80.f, 30.f);
...
void CUIButtonWindow::SoundChangeCallback()
{
	...
	TCHAR	filter[] = TEXT("모든 파일\0*.*\0WAV\0*.wav\0MP3\0*.mp3\0OGG\0*.ogg");
	ofn.lStructSize = sizeof(OPENFILENAME);
	ofn.hwndOwner = CEngine::GetInst()->GetWindowHandle();
	ofn.lpstrFilter = filter;
